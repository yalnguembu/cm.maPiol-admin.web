...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    ...
    const response = await userServices.getUserById(profile?.id ?? "");
  ...
  const EditProfileModal = ({user}) => {
    ...
    const [cni, setCNI] = useState<string>(user.num_cni);
    ...
    const onSubmit = async (data) => {
      try {
        const user = await userServices.updateUser(profile.id, {
          email,
          firstname: prenom,
          lastname: nom,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-7 gap-6">
            ...
            <div className="lg:col-span-3">
              ...
              >
                ...
                <div
                  className="absolute top-1/2 -translate-y-1/2 ltr:right-6 rtl:left-6 mt-2 h-12 w-12 bg-slate-800 text-white rounded-full text-xs font-medium flex flex-col items-center justify-center">
                  Now
                </div>
              </div>
            </div>
